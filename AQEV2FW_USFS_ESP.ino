...
#define PM_ANALOG_PIN          (A4)
#define TEMPERATURE_ANALOG_PIN (A5)
#define HUMIDITY_ANALOG_PIN    (A7)
...
float instant_temperature_volts = 0.0f;
float instant_humidity_volts = 0.0f;
float instant_pm_ugpm3 = 0.0f;
...
#define NUM_SENSOR_BUFFERS        (3)
float sample_buffer[NUM_SENSOR_BUFFERS][MAX_SAMPLE_BUFFER_DEPTH] = {0};
...
#define EEPROM_PM_CAL_SLOPE       (EEPROM_PM_SENSITIVITY - 4)     // float value, 4-bytes, the slope applied to the sensor    
...
const char cmd_string_pm_slope[] PROGMEM    = "pm_slope   ";
...
void initializeHardware(void) {
  ...
  Serial.println(F(" |        USFS Sensor Suite           |"));  
  ...
  updateLCD("US FOREST", 0);
  updateLCD("SERVICE MODEL", 1);
  SUCCESS_MESSAGE_DELAY();
...
void help_menu(char * arg) {
  ...
  if (arg == 0) {
    ...
    if (strncmp("help", arg, 4) == 0) {
      ...
      get_help_indent(); Serial.println(F("pm_off - PM sensor intercept [V]"));
      get_help_indent(); Serial.println(F("pm_slope - PM sensor slope [V/V]"));
      ...
      get_help_indent(); Serial.println(F("pm_blv - the particulate baseline voltage characterization"));
      ...
      get_help_indent(); Serial.println(F("pm         - restores the Particulate calibration parameters from BACKUP "));
      ...
      get_help_indent(); Serial.println(F("pm       - backs up the Particulate calibration parameters"));    
      ...
      Serial.println(F("   <number> is the decimal value of particulate sensor intercept [ug/m^3]"));
    }
    else if (strncmp("pm_slope", arg, 8) == 0) {
      Serial.println(F("pm_slope <number>"));
      Serial.println(F("   <number> is the decimal value of particulate sensor slope [(ug/m^3)/V]"));
    }    
...
void print_eeprom_value(char * arg) {
  if (strncmp(arg, "mac", 3) == 0) {
  ...
  else if (strncmp(arg, "pm_slope", 8) == 0) {
    ...
    print_label_with_star_if_not_backed_up("Particulate Offset [ug/m^3]: ", BACKUP_STATUS_PM_CALIBRATION_BIT);
    print_eeprom_float((const float *) EEPROM_PM_CAL_OFFSET);
    print_label_with_star_if_not_backed_up("Particulate Slope [(ug/m^3)/V]: ", BACKUP_STATUS_PM_CALIBRATION_BIT);
    print_eeprom_float((const float *) EEPROM_PM_CAL_SLOPE);    
    Serial.print(F("    ")); Serial.println(F("Particulate Baseline Voltage Characterization:"));
...
void restore(char * arg) {
  ...
  if (strncmp(arg, "defaults", 8) == 0) {
    ...
    eeprom_write_block("aqev2_usfs_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
  }  
  else if (strncmp("key", arg, 3) == 0) {
...
boolean publishHeartbeat(){
  ...
  "\"converted-value\":%d,"
  "\"firmware-version\":\"%s\","
  "\"publishes\":[\"particulate\",\"temperature\",\"humidity\"],"
  "\"counter\":%lu"
...
void temperature_convert_to_degC(float average, float * converted_value){
  *converted_value = average * 20.0f;
}

void humidity_convert_to_percent(float average, float * converted_value){
  *converted_value = average * 20.0f;
}
...
boolean publishTemperature(){
  ...
  temperature_convert_to_degC(temperature_moving_average, &temperature_degc);
  float raw_temperature = temperature_moving_average;
  ...
  if(temperature_units == 'F'){
    ...
    safe_dtostrf(toFahrenheit(instant_temperature_volts), -6, 4, raw_instant_value_string, 16);
  }
  else{
    safe_dtostrf(instant_temperature_volts, -6, 4, raw_instant_value_string, 16);
  }
  safe_dtostrf(reported_temperature, -6, 2, converted_value_string, 16);
  safe_dtostrf(raw_temperature, -6, 4, raw_value_string, 16);
...
boolean publishHumidity(){
  ...
  humidity_convert_to_percent(humidity_moving_average, &relative_humidity_percent);
  float raw_humidity = humidity_moving_average;
  ...
  safe_dtostrf(raw_humidity, -6, 4, raw_value_string, 16);
  safe_dtostrf(instant_humidity_volts, -6, 4, raw_instant_value_string, 16);
...
void collectTemperature(void){
  burstSampleADC(TEMPERATURE_ANALOG_PIN, &instant_temperature_volts);
  addSample(TEMPERATURE_SAMPLE_BUFFER, instant_temperature_volts);   
...
void collectHumidity(void){
  burstSampleADC(HUMIDITY_ANALOG_PIN, &instant_humidity_volts);
  addSample(HUMIDITY_SAMPLE_BUFFER, instant_humidity_volts);   
  if(sample_buffer_idx == (sample_buffer_depth - 1)){
    humidity_ready = true;
  }    
}
...
void addSample(uint8_t sample_type, float value){
  if((sample_type < NUM_SENSOR_BUFFERS) && (sample_buffer_idx < MAX_SAMPLE_BUFFER_DEPTH)){
...
void collectPM(void){    
  burstSampleADC(PM_ANALOG_PIN, &instant_pm_ugpm3);
  addSample(PM_SAMPLE_BUFFER, instant_pm_ugpm3);   
  if(sample_buffer_idx == (sample_buffer_depth - 1)){
    pm_ready = true;
  }
}
...
void pm_convert_to_ugpm3(float average, float * converted_value, float * temperature_compensated_value){
  static boolean first_access = true;
  static float pm_zero_volts = 0.0f;
  static float pm_slope = 0.0f;
  
  if(first_access){   
    pm_zero_volts = eeprom_read_float((const float *) EEPROM_PM_CAL_OFFSET);
    pm_slope = eeprom_read_float((const float *) EEPROM_PM_CAL_SLOPE);
    first_access = false;
  }
  ...
  *converted_value = average * pm_slope + pm_zero_volts; 
...
boolean publishPM(){
  ...
  pm_convert_to_ugpm3(pm_moving_average, &converted_value, &compensated_value);
  pm_ugpm3 = compensated_value;  
  ...
  safe_dtostrf(instant_pm_ugpm3, -8, 5, raw_instant_value_string, 16);
...
void printCsvDataLine(){
  ...
  if(pm_ready){
    ...
    Serial.print(pm_ugpm3, 2);
    appendToString(pm_ugpm3, 2, dataString, &dataStringRemaining);    
  }
  else{
    Serial.print(F("---"));
    appendToString("---", dataString, &dataStringRemaining);
